...
## Solutions
so you creates a variable name `endPoison` which shows the end of the poison effect. in each iteration you check if the `endPoison` is less than the next item or not. if it is it means the poison effects will not be reset. so you add `duration` to the result. otherwise you add the difference between the next item and the current item.