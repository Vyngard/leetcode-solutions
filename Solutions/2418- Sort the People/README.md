...
## Solutions
it's easy. you should create a map(dictionary) to store the name and height of each person, then sort the height in descending order. and then create a new list, iterate over the sorted height and add the name of each height to the new list. the time complexity of this solution is O(nlogn) because of sorting the height and the space complexity is O(n) because of the new list and the map.
