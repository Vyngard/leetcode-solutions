...
You just have to iterate in linkedlist and for each node, check if the value is equal to the value of the next node. if it is, then delete the next node, and connect the current node to the next next node. and then do this again. otherwise move to the next node.