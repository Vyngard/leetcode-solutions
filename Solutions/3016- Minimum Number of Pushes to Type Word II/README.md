...
This algorithm calculates the minimum number of button pushes required to type a word on a phone keypad. It uses a frequency-based approach:

1. Count the frequency of each character in the word.
2. Sort characters by frequency in descending order.
3. Assign characters to buttons (8 per button), with more frequent characters on earlier buttons.
4. Calculate total pushes: sum of (character frequency * button push count).

This method optimizes button assignments to minimize total pushes, especially effective for longer words with varied character frequencies.

Time Complexity: `O(k log k)`, where k is the number of unique characters in the word. 
In the worst case, this becomes `O(n log n)` where n is the length of the word. 
However, for typical English words, k is usually bounded, making the practical 
complexity closer to `O(n)`.

Space Complexity: `O(k)`, which in the worst case is `O(n)` if all characters are unique.
