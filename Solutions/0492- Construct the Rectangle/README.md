...
## Solutions
so the question basically asks you to find the two numbers that are closest to each other and their product is equal to the given number. you can find the mean of the number using `sqrt()` and the subtract by 1 to find the first number. just return that number and the given number divided by it.