...
we traverse each element in the matrix, and set the element at this position to `x = min(rowSum[i], colSum[j])`, then we subtract `x` from `rowSum[i]` and `colSum[j]`, and continue to the next element. If we can't find a valid `x`, we return an empty list.
