...
## Solutions
so we sort the array first, and in each round we retrieve the last value (max value) of the array. also we need to check if the value is less than 0, we choose 0 instead (that's why we use the `max()` function). then in each rount we sum the the max value to the answer and return it. remember to decreament the value of the last element in the array by 1 in each round. the time complexity of this solution is O(nlogn) because of the sorting.
