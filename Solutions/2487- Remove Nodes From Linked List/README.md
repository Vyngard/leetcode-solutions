...
## Solutions
first we consider a stack. then we iterate the linked list. if the current node is greater than the top of the stack, we pop the stack until the top of the stack is greater than the current node, or the stack is empty. then we push the current node into the stack. we do this for all the elements in the linked list. finally, we reverse the stack and return the result.