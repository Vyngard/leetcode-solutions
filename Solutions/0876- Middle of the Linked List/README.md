...
## Solutions
we use the `fast` and `slow` solution. `fast` moves two steps at a time, and `slow` moves one step at a time. When `fast` reaches the end of the list, `slow` will be at the middle of the list. the trick is to check if the `fast.next != null` then we move the `slow` pointer to the next node. 