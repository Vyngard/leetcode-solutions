...
## Solutions
you can use two pointers to solve this problem. The idea is to keep track of the frequency of each element in the subarray and if the frequency of any element exceeds the limit `k`, then we need to shrink the window from the left side until the frequency of all elements in the window is less than or equal to `k`. The length of the window will be the maximum length of the good subarray.