...
## Solutions
you can use array to solve this problem. you make two array. one `following` which store the number of people that someone trusts, and `follower` which store the number of people that trust that person. then you can iterate through the array and check if the person is trusted by `n-1` people and trust no one. if you find such person then return the index of that person. if you don't find such person then return `-1`.