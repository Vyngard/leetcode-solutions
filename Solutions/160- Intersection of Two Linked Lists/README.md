...
the easier way is to travel both linkedlists separately and find their length. then the move the longer linkedlist pointer forward, until it reaches to a place that is the same length as the shorter linkedlist. then travel both linkedlists together and find the intersection node.    
But we can do better. So imagine the first linkedlist hast 2 nodes and the second one has 3 node. if we iterate the first linkedlist and then after we finished, we continue and iterate the second list too, in the end we have the length of 5 (2 + 3). the same thing happens if we iterate the second list and the continue on the first one. so if we do that simoltaniously, for both pointers, at one level they will be at the same place. if these two linkedlists have intersection, they will meet at the intersection node. if they don't have intersection, they both will be null in the end.