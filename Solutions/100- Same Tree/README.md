...
## Solutions
You check each node one by one to see if they are the same. If they are not the same, return false. for base case, you should be careful, since one of them could be null, so if you face that again return `false`. if both of them are `null`, then it means you reach the end of the tree without any problem, so return `true`.