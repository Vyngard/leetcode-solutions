...
## Solutions
it is easy. just check each number from `left` to `right` if it is self-dividing number. for checking if a number is self-dividing number, we extract each digit of the number and check if the number is divisible by the digit. if any digit is zero or the number is not divisible by the digit, then the number is not self-dividing number.
