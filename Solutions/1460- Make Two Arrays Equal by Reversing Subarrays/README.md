...
you can use dictionary (hash table) to count the frequency of each number in the `arr`. then iterate through the `target`. if the item in the `target` is not in the dictionary, return `False`. otherwise return `True` at the end.